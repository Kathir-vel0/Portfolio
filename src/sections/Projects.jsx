        ...
        {projects.map((project, index) => (
          ...
          >
            <div className="flex flex-col items-center text-center mb-16 gap-4">
              <p className="text-[#a0e8f5] text-[10px] font-bold uppercase tracking-[0.2em] mb-2">MAIN PROJECT</p>
              <h2 className="text-4xl sm:text-6xl md:text-7xl font-bold tracking-tighter text-white uppercase break-all drop-shadow-[0_0_15px_rgba(160,232,245,0.1)]">
                {project.title}
              </h2>
            </div>

            <div className="relative group bg-black/30 backdrop-blur-xl rounded-2xl overflow-hidden border border-white/10 shadow-[0_0_20px_rgba(160,232,245,0.05)] hover:shadow-[0_0_40px_rgba(160,232,245,0.15)] transition-all duration-700">
              <div className="aspect-[16/9] md:aspect-[21/9] w-full relative overflow-hidden">
                ...
                <div className="absolute bottom-0 left-0 p-6 sm:p-10 z-20 w-full flex flex-col md:flex-row md:items-end justify-between gap-8">
                  <div className="max-w-2xl bg-black/40 backdrop-blur-md p-6 rounded-xl border border-white/5">
                    <div className="flex flex-wrap gap-2 mb-6">
                      {project.tags.map(tag => (
                        <span key={tag} className="px-3 py-1.5 bg-[#a0e8f5]/10 border border-[#a0e8f5]/30 text-[9px] sm:text-[10px] uppercase tracking-[0.2em] text-[#a0e8f5]">
                          {tag}
                        </span>
                      ))}
                    </div>
                    ...
                    <div className="flex flex-wrap gap-4 mt-6">
                      {project.stack.map(tech => (
                        <span key={tech} className="text-white/80 text-[10px] font-mono font-bold tracking-widest uppercase bg-white/5 px-2 py-1 rounded">
                          // {tech}
                        </span>
                      ))}
                    </div>
                  ...
                  <div className="flex flex-wrap gap-4 mt-6 md:mt-0">
                    {project.demo !== "#" && (
                      <a href={project.demo} target="_blank" rel="noreferrer" className="flex items-center gap-3 px-6 py-3 rounded-xl bg-white/5 border border-white/20 backdrop-blur-sm text-white hover:bg-[#a0e8f5] hover:border-[#a0e8f5] hover:text-black hover:shadow-[0_0_20px_rgba(160,232,245,0.5)] transition-all shrink-0 text-[10px] sm:text-xs font-bold uppercase tracking-widest group/btn">
                    ...
                    <a href={project.github} target="_blank" rel="noreferrer" className="flex items-center gap-3 px-6 py-3 rounded-xl bg-[#a0e8f5]/10 border border-[#a0e8f5]/30 backdrop-blur-sm text-[#a0e8f5] hover:bg-[#a0e8f5] hover:text-black hover:shadow-[0_0_20px_rgba(160,232,245,0.5)] transition-all shrink-0 text-[10px] sm:text-xs font-bold uppercase tracking-widest group/btn">
